...
- **Claude Code plugin**: `.claude-plugin/plugin.json` declares the MCP server with `${CLAUDE_PLUGIN_ROOT}` paths and sets `WHATSAPP_PLUGIN_MODE=1`; `.claude-plugin/marketplace.json` makes this repo its own marketplace (`pachperdev-tools`, plugin `whatsapp`, source `./`). Install: `/plugin marketplace add mauricioDevApp/whatsapp-mcp-plugin` + `/plugin install whatsapp@pachperdev-tools`.
- **Plugin mode** (`WHATSAPP_PLUGIN_MODE=1`): all mutable data moves to `~/.whatsapp-mcp/` (store, compiled binary under `bin/`, logs) so plugin updates never destroy the session. The supervisor auto-builds the bridge binary with Go (`ensure_bridge_binary` in `bridge.py`) when missing.
